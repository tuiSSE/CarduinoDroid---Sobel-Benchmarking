...
public class Camera_Settings {
	...
	public void send_change_filter(boolean useEdgeDetection) {
		String useEdge ="0";
		if(useEdgeDetection){
			useEdge = "1";
		}
		if  (controller_computer.network.send_camera_settings("5;" + useEdge)) {
			controller_computer.log.writelogfile("Edge Detection " + useEdge);
		} else {
			controller_computer.log.writelogfile("Edge Detection " + useEdge);
		}
	}
